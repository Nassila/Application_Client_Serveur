...
public class Outils {
	...
	public static String getAnnonces() {
		...
		try {
			...
			if(c.resultSet != null) {
				...
				c.toSend = "CONFIRMED/" + c.getAnn.substring(0, c.getAnn.length() - 1) ;//envoie les annonces sauf le dernier "|"
	...
	public static String getMyAnnonces(int id) {
		...
		try {
			...
			if( c.resultSet != null) {
				...
				c.toSend = "CONFIRMED/" + c.getMyAnn.substring(0, c.getMyAnn.length() - 1) ;//envoie les annonces sauf le dernier "|"
	...
	public static String newAnnonce(String nom, String domaine, double prix, String des, int id) {
		...
		try {
			...
			//Requête SQL
			c.sqlRequest = "INSERT INTO `annonces` (`nom`, `domaine`, `prix`, `descriptif`, `refCL`) VALUES ('"
						+ c.nom + "', '" + c.domaine + "', '" + c.prix + "', '" + c.descriptif + "' , '" + id + "');";

			//Connexion avec la base de données
			c.preparedState = Connexion.connexionBD().prepareStatement(c.sqlRequest,
	...
	public static String deleteAnnonce(int codeDel, int id) {
		  ...
		  Config c = new Config();
	    	...
	    	try {
				...
				while (c.resultSet.next()){//Parcours de la base de données
					...
					if ((c.loginBdd.equals(login)) && (c.passwordBdd.equals(passWord)) ) {//Test sur le login et le mot de passe saisis
				...
				if (c.connexionSuccess.equals("CONNECTED")) {//Si le client est connecté, la while de validité est terminée
					...
					return (c.connexionSuccess+"/"+c.name+","+c.prenom+","+login+","+passWord+"/"+c.getAnn+"-"+idc);
				}
	  ...
	  public static String newuser(String name, String prenom, String login, String password ) {
		  ...
		  try {
				...
				//Requête SQL
				c.sqlRequest = "INSERT INTO `clients` (`nom`, `prenom`, `username`, `password`) VALUES ('"+ name + "', '" +prenom + "', '" + login + "', '" + password +"');";
				...
				return ("CONFIRMED"+"/"+name+","+prenom+","+login+","+password+"/"+c.getAnn+"-"+idc);
